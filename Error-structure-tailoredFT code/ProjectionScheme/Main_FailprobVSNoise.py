...
Output_Filename = 'MainProjFailprobVSNoise251016.pkl'   #!!! name of the output data file
...
def main():
    ...
    with multiprocessing.Pool(processes = min(ncpus, len(scanner))) as pool:
        ...
        for id, p_dep in enumerate(scanner):
            ...
            for m in [2, 3]:
                param_list.append((p_dep, m, num_shots))
                if m == 2:
                    processes.append(
                        pool.apply_async(
        ...
        results_dict = {}
        for (p_dep, m, num_shots), process in zip(param_list, processes):
            try:
                ...
                print(f"Error processing p_dep={p_dep}, m={m}: {str(e)}")
        ...
        for p_dep in results_dict:
            for m in results_dict[p_dep]:
                data = results_dict[p_dep][m]
    ...
    try:
        for (p_dep, m, _) in param_list:
            shm_name = f"bk_{p_dep}_{m}"
    ...
    plt.xlabel('Physical error rate p', fontsize=16)  # Increased from 12 to 16
    ...
    # Save the plot
    plt.savefig('Proj_PfailVSNoise_251016Test.eps', format='eps', dpi=1000, bbox_inches='tight')
    plt.show()
