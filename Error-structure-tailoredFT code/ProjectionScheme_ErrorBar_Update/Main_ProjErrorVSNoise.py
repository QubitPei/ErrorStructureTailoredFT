...
Output_Filename = 'MainProjectionNoise260109_Errorbar.pkl'   #!!! name of the output data file
...
def Proj_Count_naive(rounds: int,  # number of rounds for the syndrome measurement, should >= rounds_post
    ...
    for bk, prob_bk in sorted(bk_probabilities.items()):
        ...
        num_bk_fail = count_logical_failure_Projection_parallel(circuit, Post_Dec_List, int(Num_shots))
...
def main():
        ...
        for p_dep in results_dict:
            for ZZtype in results_dict[p_dep]:
                ...
                if data['frequencies']:  # Only process valid data
                    Tr_Dist, P_fail = util2.Trace_distace_estimate(
                        phi=phi,
    ...
    # Save the plot
    plt.savefig('Projection_Pfail_260109.eps', format='eps', dpi=1000, bbox_inches='tight')
    # plt.show()
    ...
    plt.yscale('log')
    plt.xlabel('Physical Error Rate (p)', fontsize=12)
    plt.ylabel('Trace distance $D_{Tr}$', fontsize=12)
    plt.grid(True, which="both", ls="-", alpha=0.6)
    plt.axvline(x=1e-3, color=colors_trace['hw_line'], linestyle=':', label='current hardware')
    plt.legend(fontsize=10, framealpha=0.8)

    # Save the plot
    plt.savefig('Projection_ErrorRate_260109.eps', format='eps', dpi=1000, bbox_inches='tight')
    # plt.show()
    ...
    Result_Dep_List = np.column_stack((Tr_Dist_Dep, Fail_Prob_Dep, Std_Tr_Dist_Dep))  # Trace distance, Failure prob. for naive depelorizing ZZ
    Result_Disp_List = np.column_stack((Tr_Dist_Disp, Fail_Prob_Disp, Std_Tr_Dist_Disp))   # Trace distance, Failure prob. for dispersive ZZ
