...
def sample_and_count_failures(shots: int, circuit: stim.Circuit, Post_Dec_List: list[int]) -> int:
    sampler = circuit.compile_detector_sampler()
    # 进行采样
    detection_events, _ = sampler.sample(shots, separate_observables=True)
    
    # 在 Worker 内部直接进行后选择 (Post-selection) 逻辑
    selected_columns = detection_events[:, Post_Dec_List]
    abandoned_rows = np.any(selected_columns, axis=1)
    
    # 仅返回失败的次数 (int)，而非整个数组
    return int(np.sum(abandoned_rows))
...
def Error_bar_estimate(phi:float= 0, z_distance: int= None, 
                           Num_shots: int= None, bk_probabilities: dict[str,float] = None, 
                           bk_fail_shots: Counter[str] = None) -> float:
    # To estimate the error bar of the trace distance estimation
    # Analytical estimation based on the delta method of ratio estimator

    # Step 1: probability estimator: P_{bk,pass}
    P_bk_pass: dict[str, float] = {}
    Ppass = 0
    P_ud_pass = 0
    for bk, num_bk in sorted(bk_probabilities.items()):
    ...
    # output: Ppass, P_ud_pass

    # Step 2: variation estimator for Ppass and P_ud_pass
    Vars_Ppass = 0
    Vars_P_ud_pass = 0
    for bk, num_bk in sorted(bk_probabilities.items()):
        P_pass_con_bk = ( 1 - (bk_fail_shots[bk]/Num_shots) )
        Vars_P_pass_con_bk = P_pass_con_bk*(1-P_pass_con_bk)/(Num_shots-1)
        Vars_Ppass += (bk_probabilities[bk]**2) * Vars_P_pass_con_bk
        if any(bit == '1' for bit in bk):  # non-zero string
            Vars_P_ud_pass += (bk_probabilities[bk]**2) * Vars_P_pass_con_bk

    # Step 3: Delta method to estimate the variance of ratio
    A = P_ud_pass
    B = Ppass
    V_A = Vars_P_ud_pass
    V_B = Vars_Ppass
    Vars_R = (V_A/(B**2))*(1 - (2*A/B)) + (A**2/B**4)*V_B

    # Step 4: Estimate the std-error, error bar for Tr_Dist estimator
    # Tr_Dist based on the Bayesian probability: P_{ud|pass}
    k = int(z_distance/2)
    # theta = phi**(1/k)
    theta = solve_theta(phi, k, delta=1e-14)
    ...
    Std_Tr_Dist = np.abs(np.sin(phi-phi_prime))*np.sqrt(Vars_R)
...
def Error_bar_estimate_m3(phi:float= 0, z_distance: int= None, 
                           Num_shots: int= None, bk_probabilities: dict[str,float] = None, 
                           bk_fail_shots: Counter[str] = None) -> float:
    # To estimate the error bar of the trace distance estimation
    # Analytical estimation based on the delta method of ratio estimator
