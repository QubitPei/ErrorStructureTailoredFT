...
fig_name = 'Projection_ErrorScale_???.eps'
...
def Proj_Count_naive_m3(rounds: int,  # number of rounds for the syndrome measurement, should >= rounds_post
...
def main(name):
    ...
    for id, k in enumerate(k_List):
        ...
        ZZtype = 1
        bk_frequencies, bk_fail_shots = Proj_Count_naive_m3(rounds = rounds,  # number of rounds for the syndrome measurement, should >= rounds_post
        ...
        Tr_Dist, P_fail = util2.Trace_distace_estimate_m3(phi = phi, z_distance = z_distance, 
                            Num_shots = num_shots, bk_probabilities = bk_frequencies, 
                            bk_fail_shots = bk_fail_shots)
        Std_Tr_Dist = util2.Error_bar_estimate_m3(phi = phi, z_distance = z_distance, 
                            Num_shots = num_shots, bk_probabilities = bk_frequencies, 
                            bk_fail_shots = bk_fail_shots)
    ...
    plt.ylabel(r'$c(k)=P_{\mathrm{ud}(2)}/(p^2\cdot |\varphi|)$', fontsize=16)
...
if __name__ == '__main__':
 #   multiprocessing.freeze_support()  # For Windows executable support
    for i in range(10):
        name = './data/MainProjScale251219m3_' + str(i)   #!!! name of the output data file
        main(name)